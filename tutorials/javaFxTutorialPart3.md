...
For duke.user.DukeUI, there are two events that we want to respond to, namely the user pressing `Enter` in the `TextField` and left-clicking the `Button`. These are the `onAction` event for the `TextField` and the `onMouseClicked` event for the `Button`.
...
Let’s create our custom control `duke.user.DialogBox`:
```java
...
public class duke.user.DialogBox extends HBox {
    ...
    public duke.user.DialogBox(Label l, ImageView iv) {
        text = l;
        displayPicture = iv;
...
For this tutorial, we have two images `DaUser.png` and `DaDuke.png` to represent the user avatar and duke.user.DukeUI's avatar respectively but you can use any image you want.
...
```java
public class duke.user.DukeUI extends Application {
    // ...
...
private void handleUserInput() {
    ...
    dialogContainer.getChildren().addAll(
            new duke.user.DialogBox(userText, new ImageView(user)),
            new duke.user.DialogBox(dukeText, new ImageView(duke))
    );
    userInput.clear();
}
...
private String getResponse(String input) {
    return "duke.user.DukeUI heard: " + input;
}
```
...
## Iteration 3 – Adding custom behavior to duke.user.DialogBox

One additional benefit of defining a custom control is that we can add behavior specific to our `duke.user.DialogBox`. Let’s add a method to flip a dialog box such that the image on the left to differentiate between user input and duke.user.DukeUI’s output.
...
public static duke.user.DialogBox getUserDialog(Label l, ImageView iv) {
    return new duke.user.DialogBox(l, iv);
}

public static duke.user.DialogBox getDukeDialog(Label l, ImageView iv) {
    var db = new duke.user.DialogBox(l, iv);
    db.flip();
    return db;
}
```
...
Now, we can go back to the `Main` class and change the event handler to use our new `duke.user.DialogBox`. 
...
private void handleUserInput() {
    ...
    Label dukeText = new Label(getResponse(userInput.getText()));
    dialogContainer.getChildren().addAll(
            duke.user.DialogBox.getUserDialog(userText, new ImageView(user)),
            duke.user.DialogBox.getDukeDialog(dukeText, new ImageView(duke))
    );
    userInput.clear();
}
```
...
Congratulations! 
You have successfully implemented a fully functional GUI for duke.user.DukeUI!  
...
1. While the GUI looks similar to the mockup, there are still parts that need to be refined. Try your hand at some of these tasks:
   * Add padding between each duke.user.DialogBox
