...
1. Components are heavily dependent on each other:
   Why does `Main` need to know that `duke.user.DialogBox` needs a `Label`? 
...
Let's return to duke.user.DukeUI and convert it to use FXML instead.
...
<AnchorPane maxHeight="-Infinity" maxWidth="-Infinity" minHeight="-Infinity" minWidth="-Infinity" prefHeight="600.0" prefWidth="400.0" xmlns="http://javafx.com/javafx/8.0.171" xmlns:fx="http://javafx.com/fxml/1" fx:controller="duke.user.MainWindow">
   <children>
      <TextField fx:id="userInput" layoutY="558.0" onAction="#handleUserInput" prefHeight="41.0" prefWidth="324.0" AnchorPane.bottomAnchor="1.0" />
...
1. Let’s explore the provided FXML files in Scene Builder. 
    ...
    Select `Open Project` > `src/main/resources/view/duke.user.MainWindow.fxml`. Inspect each control and its properties.

   ![SceneBuilder opening duke.user.MainWindow.fxml](assets/SceneBuilder.png)
...
1. On the left accordion, you can see that we have set the controller class to `duke.user.MainWindow`. 
We will get to that later.
 
   ![Controller for duke.user.MainWindow](assets/MainWindowController.png)

1. Let’s repeat the process for `duke.user.DialogBox`.
   The main difference here is that duke.user.DialogBox checks `Use fx:root construct` and _does not define a controller class_. 

   ![Settings for duke.user.DialogBox](assets/DialogBoxController.png)

## Using Controllers

As part of the effort to separate the code handling duke.user.DukeUI's logic and UI, let's _refactor_ the UI-related code to its own class.
We call these UI classes _controllers_. 

Let's implement the `duke.user.MainWindow` controller class that we specified in `duke.user.MainWindow.fxml`.

**duke.user.MainWindow.java**
```java
 ...
 */
public class duke.user.MainWindow extends AnchorPane {
    @FXML
    ...
    public void setDuke(duke.user.DukeUI d) {
        duke = d;
    }

    /**
     * Creates two dialog boxes, one echoing user input and the other containing duke.user.DukeUI's reply and then appends them to
    ...
    private void handleUserInput() {
        ...
        dialogContainer.getChildren().addAll(
                duke.user.DialogBox.getUserDialog(input, userImage),
                duke.user.DialogBox.getDukeDialog(response, dukeImage)
        );
        userInput.clear();
    }
}
...
Notice how in `duke.user.MainWindow`, we can invoke `TextField#clear()` on `userInput` and access its content just as we did in the previous example.
...
Let's create a new `Main` class as the bridge between the existing logic in `duke.user.DukeUI` and the UI in `duke.user.MainWindow`.
...
/**
 * A GUI for duke.user.DukeUI using FXML.
 */
public class Main extends Application {

    private duke.user.DukeUI duke = new duke.user.DukeUI();

    @Override
    public void start(Stage stage) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(Main.class.getResource("/view/duke.user.MainWindow.fxml"));
            AnchorPane ap = fxmlLoader.load();
            Scene scene = new Scene(ap);
            stage.setScene(scene);
            fxmlLoader.<duke.user.MainWindow>getController().setDuke(duke);
            stage.show();
        } catch (IOException e) {
            e.printStackTrace();
        }
...
For our custom `duke.user.DialogBox`, we did not define a controller so let's create a controller for it.

**duke.user.DialogBox.java**
```java
import java.io.IOException;
 ...
 */
public class duke.user.DialogBox extends HBox {
    @FXML
    private Label dialog;
    @FXML
    private ImageView displayPicture;

    private duke.user.DialogBox(String text, Image img) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(duke.user.MainWindow.class.getResource("/view/duke.user.DialogBox.fxml"));
    ...
    public static duke.user.DialogBox getUserDialog(String text, Image img) {
        return new duke.user.DialogBox(text, img);
    }

    public static duke.user.DialogBox getDukeDialog(String text, Image img) {
        var db = new duke.user.DialogBox(text, img);
        db.flip();
        return db;
    }
}
...
When we create a new instance of `duke.user.DialogBox`, we set both the controller and root Node to `duke.user.DialogBox`. 
From this point onwards we can interact with `duke.user.DialogBox` as we have in the previous tutorials.
...
1. Convert `duke.user.MainWindow` to use the `fx:root` construct.
1. Extend `duke.user.MainWindow` to have a `Stage` as a root Node.
